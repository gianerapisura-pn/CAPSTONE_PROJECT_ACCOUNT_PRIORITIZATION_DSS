...
function LoginForm() {
  ...
  useEffect(() => { if (!loading && user) router.replace(nextPath) }, [loading, nextPath, router, user]);
  async function submit(event: FormEvent) {
    event.preventDefault(); setBusy(true); setError("");
    try { await signIn(email, password); router.replace(nextPath) }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Sign-in failed.") }
    finally { setBusy(false) }
  }
  return <main className="login-page"><section className="login-brand-panel"><div className="login-brand"><div className="brand-mark large"><Boxes /></div><div><strong>PESLC</strong><span>Account Prioritization DSS</span></div></div><div className="login-message"><span className="eyebrow light">Management decision support</span><h1>Historical evidence, organized for the next account review.</h1><p>Rank previous and existing accounts for management attention using transparent descriptive, predictive, and prescriptive analytics.</p></div><div className="security-note"><ShieldCheck /><div><strong>Controlled analytical workspace</strong><span>Secure role-based access, private data processing, and protected analysis history.</span></div></div></section>
