...
def classification_metrics(actual, predicted) -> dict:
    return _metrics(actual, np.asarray(predicted))


def _window_frame(
    groups: list[InvoiceGroup], window: int, config: AnalyticsConfig,
    cutoffs: list[pd.Timestamp] | None = None,
) -> pd.DataFrame:
    common_cutoffs = cutoffs or _candidate_cutoffs(groups, window, config.predictive_lookback_months)
...
def _select_window(groups: list[InvoiceGroup], config: AnalyticsConfig) -> tuple[int, pd.DataFrame, list[dict]]:
    ...
    common_cutoffs = _candidate_cutoffs(
        groups, max(config.candidate_outcome_windows), config.predictive_lookback_months
    )
    for window in config.candidate_outcome_windows:
        frame = _window_frame(groups, window, config, common_cutoffs)
        if frame.empty:
        ...
        counts = development["inactivity_risk"].value_counts()
        evidence.append({"window_months": window, "status": "viable" if viable else "insufficient development support",
                         "observations": len(frame), "development_observations": len(development),
                         "lower_count": int(counts.get(LOWER_RISK, 0)), "higher_count": int(counts.get(HIGHER_RISK, 0)),
                         "macro_f1": score})
...
def _tune(train: pd.DataFrame, validation: pd.DataFrame, features: list[str], config: AnalyticsConfig) -> tuple[dict, float]:
    ...
    for depth in config.cart_max_depth:
        for leaf in (1, 2):
            params = {"max_depth": depth, "min_samples_split": 4, "min_samples_leaf": leaf, "ccp_alpha": 0.0}
            model = _pipeline(params, config.random_seed)
...
def run_cart_analysis(
    ...
    if not window or frame.empty:
        ...
        return finish(CartResult(**{**asdict(result), "candidate_window_evidence": window_evidence}))
    cutoffs = sorted(pd.Timestamp(value) for value in frame["cutoff_date"].unique())
    ...
    permutation_std = dict(zip(CANDIDATE_FEATURES, map(float, permutation.importances_std), strict=True))
    ...
    dummy.fit(development[selected], development["inactivity_risk"])
    dummy_predicted = dummy.predict(oop[selected])
    majority_report = _metrics(oop["inactivity_risk"], dummy_predicted)
    majority = majority_report["accuracy"]
    ...
    result = CartResult(
        ...
        majority_baseline=majority, majority_baseline_report=majority_report,
        confusion_matrix=matrix, feature_evidence=evidence,
        redundancy_flags=redundancy, gini_importance=gini,
        permutation_importance=permutation_scores, permutation_importance_std=permutation_std,
    ...
    artifact = {
        ...
        "majority_class": str(development["inactivity_risk"].mode().iloc[0]),
...
def train_cart_temporal(development_frame: pd.DataFrame, oop_frame: pd.DataFrame, random_seed: int = 42,
    ...
    baseline.fit(development_frame[available], development_frame["inactivity_risk"])
    baseline_predicted = baseline.predict(oop_frame[available])
    report = _metrics(oop_frame["inactivity_risk"], predicted)
    baseline_report = _metrics(oop_frame["inactivity_risk"], baseline_predicted)
    return CartResult(status="Validated", outcome_window_months=0, feature_columns=available,
                      ...
                      confusion_matrix=confusion_matrix(
                          oop_frame["inactivity_risk"], predicted,
                          labels=[LOWER_RISK, HIGHER_RISK],
                      ).tolist(),
                      hyperparameters=model.named_steps["tree"].get_params(),
